...
| 盘点结果 | 处置 |
| --- | --- |
| §2.2 ① 旧买入组（扣款腿 pending） | 按**实际交割事实**处理：扣款确实已发生 → 走新模型确认基金腿（代码会把扣款腿校正为 confirmed）；未发生 → 取消该组。**不得**只把 CASH 腿批量置 confirmed（见 §5.3） |
...
| 已完成的合法 confirmed 组 | **不动**：历史 `trade_date`/金额原样保留，读侧照实返回（§6.5） |
...
2. 处置前核对「该组最早会计生效日」当天及之后是否有快照；有则**先删快照**
   （删除会按连续原则级联其后快照，须一并接受重算代价）。
3. **不得**把所有旧 CASH 腿批量置 confirmed 或批量删组——那会把「钱到底动没动」
   这一事实判断替换成机械操作，正是本次要消除的账实不符。
4. 每组处置完成后，用 §2.4 的在途行与平台可用现金做一次对账：
   `现金 + 在途 + 基金市值` 应与处置前一致（有手续费时按 §5.3 的差额解释）。
5. 全程留痕：处置人、时间、组号、依据（对账单/交割单）、前后状态。
