...
* **配对 CASH 腿的日期闸门**（#493）：买入扣款腿两个日期同为下单日 T；卖出到账腿 `trade_date = C`、`confirm_date = A`，且 **`A >= C`**、**`A` 须为交易日**（违反分别报 `INVALID_DATE_ORDER` / `NON_TRADING_DAY`；创建期则一律不接受到账信息）。**闸门有两处实现，改日期闸门须同改两处**：确认与预览共用 `resolve_cash_leg_plan`；PUT 修正 confirmed 卖出的到账日是同口径的**第二处实现**（`_update_confirmed_sell_arrival_date`：内联校验、不经 plan，参照日期与快照守卫亦不同）。跨家族的数据不变量「任一 CASH 腿恒有 `trade_date <= confirm_date`」见[现金账本](#rule-cash)。到账腿的日期形态决定读侧两口径何时重合：可用现金的流出按 `trade_date` 收口，而快照的现金增量与在途按 `confirm_date` 收口，只有 `trade_date == confirm_date` 时两者同时点（推理正文见 `position_service.py::calculate_available_cash` 的 docstring）。存量旧模型数据中可能存在不满足「买入扣款腿两日期同为 T」的残留行（基金确认日被传播进了扣款腿），盘点与处置结论见 [#493 切换 runbook](../runbooks/493-model-cutover.md) 的 #581 补盘记录。
...
| `NON_TRADING_DAY` | 422 | 目标日期不在 `trading_calendar.is_open`：调仓创建/PUT 的 `trade_date`（`validate_trade_date` 共用）、**调仓确认日 C 与到账日 A（#493，确认/预览共用 `resolve_cash_leg_plan`，PUT 修正到账日同口径另实现）**、申赎创建/PUT 的 `apply_date`、跨平台现金转移的 `transfer_date`、现金手动重估的 `update_date`（缺省取 `date.today()`）。快照生成侧同型检查抛 ValueError 而非此码 | services/trade_service.py::validate_trade_date; ::resolve_cash_leg_plan; ::_update_confirmed_sell_arrival_date; services/position_service.py::update_cash_position |
...
8. 更新端点（PUT）语义是「不传 = 不动」，**显式传 null 一律 422 `INVALID_PARAM`**（通用收口 `services/null_guard.py::reject_explicit_nulls`，#573；已接入投资人 / 产品 / 资产分类 / 申赎四个更新路径与路由 `platforms` / `data_sources`（#579），`portfolios` / `trades` / `share_change_events` 行为不同（「null = 未提供」，口径统一待 #579 决策））——清除型字段（申赎 `notes`、投资人 `phone`/`email`、分类 `description`、产品维度标签、平台 `platform_type`）在各自调用点用 `allow` 显式声明放行，专用校验器字段（产品 `product_type` → `INVALID_PRODUCT_TYPE`、`confirm_days`/`nav_lag_days`）保留各自错误码，故收口应排在专用校验之后（资产分类侧为「拒绝即零写入」刻意前置，会抢占 `INVALID_CLASSIFICATION` / `DIMENSION_RULE_CONFLICT`）。新增更新字段时不要放行 null：可空列会落 NULL，响应模型字段不接受 None 时该行此后 GET 恒 500（不可自愈）；`dimension_rules` 这类全量替换字段的 null 更会静默清空。
