...
* **配对 CASH 腿的日期不变量**（#493）：任一 CASH 腿恒有 `trade_date <= confirm_date`——买入扣款腿两个日期同为 T；卖出到账腿 `trade_date = C`、`confirm_date = A`，且 **`A >= C`**、**`A` 须为交易日**（违反分别报 `INVALID_DATE_ORDER` / `NON_TRADING_DAY`；确认、预览与 PUT 修正到账日三入口共用 `resolve_cash_leg_plan` 的同一道闸门，创建期则一律不接受到账信息）。这条不变量决定读侧两口径何时重合：可用现金的流出按 `trade_date` 收口，而快照的现金增量与在途按 `confirm_date` 收口，只有 `trade_date == confirm_date` 时两者同时点（推理正文见 `position_service.py::calculate_available_cash` 的 docstring）。存量旧模型数据中可能存在不满足「买入扣款腿两日期同为 T」的残留行（基金确认日被传播进了扣款腿），盘点与处置结论以 [#493 切换 runbook](../runbooks/493-model-cutover.md) 的 §6.3 为准（#581）。
