...
8. 更新端点（PUT）语义是「不传 = 不动」，**显式传 null 一律 422 `INVALID_PARAM`**（`services/null_guard.py::reject_explicit_nulls` 单一实现，#573）——清除型字段（申赎 `notes`、投资人 `phone`/`email`、分类 `description`、产品维度标签）在各自调用点用 `allow` 显式声明放行，专用校验器字段（产品 `confirm_days`/`nav_lag_days`）保留各自错误码。新增更新字段时不要放行 null：可空列会落 NULL，响应模型字段不接受 None 时该行此后 GET 恒 500（不可自愈）；`dimension_rules` 这类全量替换字段的 null 更会静默清空。
