...
1. **改完必须验证**：本地跑**改动影响面**的测试且绿即可，不要求全量（全量回归由 CI 兜底——PR 侧按路径裁剪 job 但**触碰的栈仍跑全量**，跨栈组合与纯时间流逝型失效由 nightly 全量兜底，见 `.github/workflows/ci.yml` 与 `docs/reference/code-review.md` §4.6；影响面圈定宁宽勿窄，如动 `snapshot_service` 应连带快照/申赎/调仓相关测试；影响面圈定程序见 `backend/AGENTS.md`「跑测试」节）+ 能说明改动影响；验证不了的改动不提交。
