...
- **PR 触发器禁止添加 paths-ignore**：docs-only PR 仍须产出 CI OK，否则 required check 永久等待（#377）。路径模式只按后缀/目录精确匹配，不能扩大到漏掉代码；合入前验证不减，完整执行策略见 ci.yml 与审查规范“合入”。
...
1. **改完必须验证**：本地跑改动影响面测试且绿，能说明影响；验证不了不提交。影响面宁宽勿窄，按后端指南的影响面表圈定，快照改动连带申赎/调仓等。全量由 CI 兜底：PR 按路径选栈，触碰的栈仍全量；跨栈组合和时间流逝型失效由 main push 保守全量及下次触栈 PR 兜底。
2. **排查/审查发现的问题只提 issue，不直接改代码**，由任务所有者决定修复方式。
3. **不引入未要求的依赖或表结构改动**；DB 变更必须同步提供 Alembic 迁移。
4. **仓库文档随代码同一次提交同步**，具体范围遵循 [AI 文档规范](docs/reference/documentation.md#doc-sync)；设计决策与方案记录进 issue 讨论。提交、推送、发布仍须相应授权。
5. 执行细节不明确时必须先询问用户；回答后仍有不明确之处，继续追问，不能自行决定。
