...
**响应模型守门**：返回 JSON 的端点必须声明 `response_model`——ORM 行不加收窄会全列直吐（#487）。未声明者逐条登记在 `tests/integration/test_response_model_guard.py` 的白名单（附理由、只减不增），并与 `backend/openapi.json` 两层互证（扫描操作全集；「`200` 带 `application/json` 媒体类型但 schema 为空壳」集合）；「可空列 ↔ 响应字段不接受 None」的静态核对与例外台账由 `tests/unit/test_response_model_nullability.py` 守门（`service_guard` 条目按字段登记守门模块，测试实调守门 + 更新路径调用点 AST 复核；未修缺口以 `known_gap` 登记并挂 follow-up issue 号）。**显式 null 收口**：已接入的更新路径（投资人 / 产品 / 资产分类 / 申赎四个服务（#573）与 `platforms` / `data_sources` 两个路由（#579））显式传 null 一律 422 `INVALID_PARAM`（`null_guard.reject_explicit_nulls`），不要为新字段放行 null；清除型字段在调用点用 `allow` 显式声明（如 `platforms.platform_type`：列可空且响应 Optional，null = 清除）；**未接入的端点口径不同**——`portfolios` / `trades` / `share_change_events` 取「null = 未提供」（`portfolio.display_config` 的 null / `{}` 是清空，见 §1.4；全仓口径统一待 #579 决策）。新增更新端点/字段须显式决定是否接入，不要据本节推测已全覆盖——见[易错陷阱](../docs/reference/business-constraints.md#易错陷阱)。
