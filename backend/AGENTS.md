...
| `app/constants/`                                    | 纯数据常量的单一事实来源：`asset_dimensions.py`（五维字典种子）、`audit_actions.py`（审计 action / resource\_type 枚举，列宽由 `tests/unit/test_audit_service.py` 守门）、`log_charset.py`（四张日志表的表级 charset/collate，模型与迁移 0014 共用，由 `tests/unit/test_migration_0014.py` 守门，理由见 §1.4）；埋点处禁止写字面量 |
...
* **`audit_service.py`（#405，跨切面）**：`audit_log` / `system_error_log` 的**唯一写入路径**，埋点一律调 `record_audit` / `record_system_error`，不直接 `db.add(AuditLog(...))`。
  ...
  - **失败响亮**：审计写不进去 → 只回滚 savepoint + stdout ERROR + 落一条 `system_error_log`（`error_type=AuditWriteFailure`，走独立 `SessionLocal`），绝不外抛也绝不静默；`record_system_error` 自身 best-effort，写失败只记 stdout。守护覆盖 **flush → 建 savepoint → Core INSERT → 回滚** 四个环节（#422）：savepoint **回滚失败**不再被裸 `pass` 吞掉，另记一条带 `exc_info` 的 ERROR，且日志与 `system_error_log` 文案按回滚结果分支表述（回滚失败 ≈ 连接已死，此时「业务事务不受影响」是假话）。`request_path` 在唯一写入路径内按 `SYSTEM_ERROR_PATH_MAX`（= 列宽 200，守门测试绑死模型）截断——HTTP 路径长度无界（h11 请求行上限 16 KB），超宽在 MySQL 严格模式下 errno 1406 会让**整条**错误记录静默消失；完整 path 由 `main.py` handler 的 stdout ERROR 行保留，二者互补。**同族的第二个静默面是字符集**（#427）：库级 utf8mb3 与连接侧 utf8mb4 不对称，自由文本含 4 字节字符（emoji / CJK 扩展 B）时 errno 1366 同样被本处的 best-effort except 吸收、整条记录丢失——已由四张日志表迁 utf8mb4 根治（§1.4），**不再走「写入侧转义」路线**（那会引入「哪些列要转义」的清单式遗漏面）。
...
* **四张日志表是库级 utf8mb3 之内的 utf8mb4 例外**（#427）：库级 charset 与排序规则刻意对齐生产 RDS（utf8mb3 / `utf8mb3_general_ci`，CI 建库注释同），而**连接侧是 utf8mb4**（`app/config.py` 连接串）——这个不对称不能靠改库级设置解决。四张表若继承库级 utf8mb3，自由文本列一旦出现 **4 字节** UTF-8 字符（emoji、CJK 扩展 B；中文是 3 字节、utf8mb3 容得下）即被 MySQL 严格模式拒绝（errno 1366 `Incorrect string value`）：`record_system_error` 是 best-effort，异常被吸收后**整条错误记录静默丢失**；`record_login_log`（`dependencies.py`）与 `routers/tasks.py` 的写入**不是** best-effort，更会外抛 500、连带登录/任务执行本身失败。处置：四个模型的 `__table_args__` 显式 `mysql_charset` / `mysql_collate`（常量单一事实来源 `app/constants/log_charset.py`，三方 drift 由 `tests/unit/test_migration_0014.py` 守门），存量库由迁移 **0014** 逐表 `ALTER TABLE … CONVERT TO CHARACTER SET utf8mb4` 转码（MySQL-only，SQLite 直接 return）。**库级 charset 与连接串均不改**；`ci.yml` 三处建库语句保持不变、只补注释说明本例外。**0014 的 downgrade 有条件跳过**：反向转 utf8mb3 在表内已存在 4 字节字符时必然失败，会让 `docs/runbooks/deploy-rollback.md` 的 `alembic downgrade` 半途而废，故先按 `LENGTH(col) - 3 * CHAR_LENGTH(col) > 0` 逐列探测（utf8mb3/utf8mb4 表下均精确：4 字节字符贡献 +1、3 字节中文为 0），有 4 字节字符则跳过该表并打 WARNING——不假装成功、不吞数据、CI 空库往返因此仍可逆（无需 `SKIP_DOWNGRADE`）。**刻意排除「写入侧转义/替换」路线**：那要维护「哪些列要转义」的清单、遗漏面不可见，且改变取证文本原貌。`nav_sync_detail`（同型 utf8mb3 库、`error_message` 亦接收外部接口原文）**不在本例外范围**，其失败形态是外抛而非静默丢日志，另行评估。
...
- **部分条件下不可逆**：**0014**（日志表 utf8mb3→utf8mb4）的 downgrade 在目标表已含 4 字节字符时跳过该表并打 WARNING（反向转码必然失败，跳过优于半途而废），故对空库/无 4 字节数据的库仍是完整可逆迁移——CI 往返可过、不需要豁免。
