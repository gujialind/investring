...
* **`snapshot_service.py`**：快照生成/重算/校验；三表固定生成顺序、级联回退与自动重确认（根 §2.6）；持仓增量累加与在途计算（根 §2.5）。入口：`generate_daily_snapshots`（单日生成）、`_delete_existing_snapshots`（删除 + 级联回退）、`recalculate_snapshots`（单一事务重算）、`auto_confirm_after_snapshot`（逐日重确认）。
  ...
  - `auto_confirm_after_snapshot(D)` 确认口径：`apply_date <= D` 的 pending 申赎、`ex_date == 下一交易日(D)` 的 pending 事件（仅父/独立记录）、跨天转移（`cross_day_pending`，**#493 起排除 `rebal_` 组**且置 confirmed 前显式拒绝非 CASH 腿 → `CASH_TRANSFER_NON_CASH_LEG` 条目）。**调仓交易一律不参与 auto_confirm**（#493 决策 6 / #471）：原 `pending_trades` 分支只命中调仓基金腿、已整体删除，到期未确认调仓由 `_check_pending_transactions` 阻断快照推进，须人工确认。
...
* **`trade_service.py`**：调仓创建/确认/取消（根 §2.5/§2.7）。配对腿同步的单一实现是 `sync_transfer_group`——**#493 起按操作矩阵分叉**，不再是「所有腿同状态、同 `trade_date`」：`cancelled` 整组回退；买入 unconfirm 保留 CASH 扣款腿 confirmed（日期金额不动）；卖出 unconfirm/取消删除 CASH 到账腿；编辑日期时（`propagate_dates=True`）买入扣款腿跟随下单日 T。任何路径都**不传播 `confirm_date`**，也**不制造 pending 的调仓 CASH 腿**。
  - **两腿状态解耦的落地口径**（#493）：买入 `create_trade` 即建 CASH sell（`status="confirmed"`、`trade_date=confirm_date=T`）；卖出创建只建基金腿（组号 `rebal_` 仍在任何 flush 之前分配），确认时由 `_apply_confirm_cash_leg` 按 `paired_cash_amount`（确认后净额）新建到账腿（`trade_date` = 基金确认日 C、`confirm_date` = 到账日 A）。买入确认**核验**既有扣款腿，不一致且扣款日未被快照消费才校正（`validate_confirm_cash_leg` 是 setattr 之前的只读前置，拒绝即零写入）。
  - **计划与写入分离**：`compute_confirm_plan` = `calculate_confirm_preview`（NAV/份额/金额，纯计算）+ `resolve_cash_leg_plan`（T/C/A 交易日与先后、C 晚于最新快照、方向闸门），**preview 与 confirm 共用**——router 的 preview 端点在 preview 里回有效现金平台/日期，`sync_nav` 只属于 confirm。
  - **组级快照保护**：`validate_group_snapshot_free`（组内任一腿会计生效日及之后有快照即拒，`extra_dates` 纳入新日期）供 update/cancel/delete/unconfirm 复用；`validate_buy_cash_with_addback` 的加回口径为 pending/confirmed 且限于查询时点已计提的扣款（详见根 §2.5）。
  - **读侧派生现金字段**：`build_paired_cash_leg_map` 按 `(portfolio_code, transfer_group)` 一次批量取反向 CASH 腿（不受 list 的 status/platform/分页筛选截断、不要求两腿同状态），供 list/get/create/update/confirm 填充 `TradeResponse.cash_platform_code` / `cash_confirm_date`。
  - `cash_transfers.py` 以 `cross_day` 字段（`schemas/cash_transfer.py`）区分当天完成与跨天到账；跨天判断（`list_cash_transfers`）**以 buy 腿为准**——`buy.status != "confirmed"` 或 `buy.confirm_date > buy.trade_date`；`confirm_cash_transfer` 确认组内所有仍为 pending 的 CASH 腿；验资按 `transfer_date` 时点（`calculate_available_cash(as_of_date=transfer_date)`，#493）。
