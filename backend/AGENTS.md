...
**响应模型守门**：返回 JSON 的端点必须声明 `response_model`——ORM 行不加收窄会全列直吐（#487）。未声明者逐条登记在 `tests/integration/test_response_model_guard.py` 的白名单（附理由、只减不增），并与 `backend/openapi.json` 的「200 无 JSON schema」集合互证；「可空列 ↔ 响应字段不接受 None」的静态核对与例外台账由 `tests/unit/test_response_model_nullability.py` 守门。
