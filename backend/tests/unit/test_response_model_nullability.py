...
# 机器复核（orm_default → 列确有 ORM 侧 default；service_guard → 服务层显式
# 拒绝 null）。行失效（对应字段已改成 Optional、或列已 NOT NULL）会被 stale
# 断言抓住。
# ============================================================================
...
EXCEPTIONS: dict[tuple[str, str], tuple[str, str]] = {
    ("Investor", "role"): (
        "orm_default",
        "ORM default='viewer'；REST 创建恒 viewer 不透传 role，应用写入不显式传 None",
    ),
    ...
    ("Product", "is_qdii"): (
        "orm_default",
        "ORM default=False；创建路径由服务层推导后显式传布尔值",
    ),
    ("Subscription", "status"): (
        "orm_default",
        "ORM default='pending'；状态流转走 confirm/cancel/unconfirm 端点，不写 None",
    ),
    ("Notification", "level"): ("orm_default", "ORM default='info'；应用写入不显式传 None"),
    ("Notification", "channel"): ("orm_default", "ORM default='in_app'；应用写入不显式传 None"),
    ("Notification", "status"): ("orm_default", "ORM default='pending'；已读流转写具体值"),
    ("ScheduledTask", "is_enabled"): (
        "orm_default",
        "ORM default=True；init_scheduled_tasks 种子与启停端点显式写布尔值",
    ),
    ("ScheduledTask", "timeout_seconds"): (
        "orm_default",
        "ORM default=300；init_scheduled_tasks 种子写显式值",
    ),
    ("Trade", "fee"): ("orm_default", "ORM default=0；创建/更新按金额推导或显式传入"),
    ("Trade", "status"): ("orm_default", "ORM default='pending'；状态流转走 lifecycle 端点"),
    ("Portfolio", "status"): ("orm_default", "ORM default='draft'；close/reactivate 写具体值"),
    ("ShareChangeEvent", "status"): (
        "orm_default",
        "ORM default='pending'；状态流转走 confirm/cancel/unconfirm 端点",
    ),
    ("SyncJob", "total"): ("orm_default", "ORM default=0；创建时初始化计数，执行中累加"),
    ("SyncJob", "done"): ("orm_default", "ORM default=0；创建时初始化计数，执行中累加"),
    ("SyncJob", "success_count"): ("orm_default", "ORM default=0；创建时初始化计数，执行中累加"),
    ("SyncJob", "failed_count"): ("orm_default", "ORM default=0；创建时初始化计数，执行中累加"),
    ("SyncJob", "skipped_count"): ("orm_default", "ORM default=0；创建时初始化计数，执行中累加"),
    ("SyncJob", "triggered_by"): ("orm_default", "ORM default='manual'；提交路径显式传触发来源"),
    ("NavSyncDetail", "synced_count"): ("orm_default", "ORM default=0；执行中累加"),
}

# 防扫描退化的下限（当前 204 个同名字段被核对）。低于此值说明配对表或字段集合被
# 静默清空，而非正常演进；正常增删模型时按实际值上调/下调。
MIN_CHECKED_FIELDS = 150
...
def _check_orm_default(column, key) -> None:
    assert column.default is not None, (
        f"{key} 声明 kind=orm_default，但列上没有 ORM 侧 default（机制不成立）"
    )


def _check_service_guard(_column, _key) -> None:
