...
// 80/85/65/81，branches 只余 0.11pp（≈0.4 条分支：新增 1 条未覆盖分支即
// 263/310=84.84%<85 变红），故四项统一 −1pp。棘轮：实测超阈值 ≥1pp 时在当次 PR 顺手
...
// 唯一残余是 tradePairs.ts:49 的 if false 路——走到该行必为双 CASH（43-44 行的基金腿
// 判定已排除其余组合），条件恒真、结构上不可覆盖，保留该冗余守卫。
