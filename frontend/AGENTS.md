...
- **选择框弹层交互一律走 `e2e/helpers.ts`**（#372）：平台/产品选择框的触发器、弹层、选项行与「无数据优雅 skip」的等待+文案由 helpers 单点持有，spec 内禁止再复制 `xpath=ancestor::div[@role="dialog"][1]` 与「环境中没有平台/产品数据」文案——三份拷贝曾各自漂移（同一批产品选项行，一处按 Tailwind `div.cursor-pointer`、两处按 `data-testid="product-option"` 定位）。**注意「首行 waitFor 失败即 skip」与「读全量后按条数 skip」是两种不同触发条件**（platform-select-search 用例 6/10 属后者），不要互相改写。**浮层（Radix Popover / 日历弹层）的开合一律经 `openPopover` / `settlePopovers`（#524）**：禁止再写裸 `trigger.click()` + 裸 `waitFor()`——Radix `DismissableLayer` 在关闭过程中仍监听 document pointerdown，会把紧接着「打开下一个浮层」的那次点击整口吞掉（CI 形态对比 `--retries=0` 连红 5 次的根因；本仓未装 `tailwindcss-animate`，弹层没有退出动画可等，同步点就是 popper 容器整棵卸载——别把归因写成动画），而裸 `waitFor()` 无上界，被吞一次就吃干整条用例的超时预算。**改选任何会进 preview query key 的输入（如调仓确认的到账日/到账平台）之后，动下一个浮层控件前必须先 `settlePreview(弹窗 scope)`（#551）**：确认类弹窗的 loading 取 `isLoading || isFetching`，而 loading 分支与内容分支是同位置的两棵不同子树，后台 refetch 会把弹窗内输入整棵重挂载、连带关掉正开着的 Radix 弹层——该同步点只补在测试侧，**组件侧的 remount 本身没修**（组件侧根治见 #551 内 2026-09-18 数据点评论的方案 A），别以为已根治。
