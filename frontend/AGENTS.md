...
- **选择框弹层交互一律走 `e2e/helpers.ts`**（#372）：平台/产品选择框的触发器、弹层、选项行与「无数据优雅 skip」的等待+文案由 helpers 单点持有，spec 内禁止再复制 `xpath=ancestor::div[@role="dialog"][1]` 与「环境中没有平台/产品数据」文案——三份拷贝曾各自漂移（同一批产品选项行，一处按 Tailwind `div.cursor-pointer`、两处按 `data-testid="product-option"` 定位）。**注意「首行 waitFor 失败即 skip」与「读全量后按条数 skip」是两种不同触发条件**（platform-select-search 用例 6/10 属后者），不要互相改写。**浮层（Radix Popover / 日历弹层）的开合一律经 `openPopover` / `settlePopovers`（#524）**：禁止再写裸 `trigger.click()` + 裸 `waitFor()`——Radix `DismissableLayer` 在关闭过程中仍监听 document pointerdown，会把紧接着「打开下一个浮层」的那次点击整口吞掉（CI 形态对比 `--retries=0` 连红 5 次的根因；本仓未装 `tailwindcss-animate`，弹层没有退出动画可等，同步点就是 popper 容器整棵卸载——别把归因写成动画），而裸 `waitFor()` 无上界，被吞一次就吃干整条用例的超时预算。**弹窗内的用户输入不得随预览 loading 分支换树路径**（#551 方案 A）：确认类弹窗的 loading 取 `isLoading || isFetching`（预览值即确认值，后台 refetch 也算加载中），`ConfirmInfoDialog` 因此把内容区做成**定长两槽**（状态分支 + `inputSlot`），三态下输入块同父同索引、跨态复用实例。曾经的拐棍 `settlePreview`（测试侧等文案消失）已随之退役——**不要**再引入同类文本谓词同步点：它是 page 外不可见的字符串契约，谓词一旦永不满足就每次静默烧满超时。新增会进 preview query key 的输入时，一律挂 `inputSlot`，不要挂回 `children`。
