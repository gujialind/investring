...
- **期望空 diff；非空 diff 硬 fail**。形态变化属预期时（增删用例/调整 skip）给 PR 打 `e2e-morph-expected` 标签——labeled 事件自动重跑 CI，compare 转为 warning 放行（长期挂在开放 PR 上会被 nightly 扫出并 warning，#472）。diff 全文在 compare job 的 Summary，raw JSON 与失败证据在 `e2e-raw-*` artifact。
