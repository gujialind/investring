...
- **分母边界与已知缺口（#505 结论：维持现状、成文接受）**：`src/hooks/**`（17 文件 / 2226 行，约为分母的 2 倍）与 `src/stores/**`（2 文件 / 146 行）**不在覆盖率分母内**，于是一行测试都没有也不会让任何门禁变红。这是记录在案、不当作 TODO 的缺口——它的存在方式是「E2E 兜底 + 人工评审」，不是「单测兜底」。两条扩张方案按实测否决（数据与推理见 `vitest.config.ts` 的「分母边界」注释）：并入会让分母 1141→3513 行、99 的阈值须整体重定；只并入 stores 在 node 下实测最高约四成（`authStore` 的落盘与 cookie 写在 `typeof window` 守卫内，zustand persist 取不到 storage 时退化为「只 warn 不落盘」），要么整体调低阈值、要么先引浏览器环境。**重新评估的可判定触发条件**：① 出现任一「根因在 hooks 内的纯计算逻辑、且 E2E 定位不到具体分支」的线上缺陷；② 决定引入 jsdom/RTL（届时本条上方的「不引 jsdom/RTL」取舍与阈值口径须同批修订）。**动作约束**：`coverage.include` 是全局阈值与增量门禁共用的分母开关，改它必须同批改 `ci.yml` 的 `LCOV_SF_MIN` 与 `scripts/tests/test_ci_frontend_coverage.py` 的基准（#509）。
...
- **守卫自身的失效面（#509）**：增量门禁的三段守卫各有独立的哑火形态，因此各有独立出口——① `Assert lcov data source (PR)` 里的**分母棘轮** `LCOV_SF_MIN`（收窄 `coverage.include` 会让覆盖率**变好**且门禁 exit 0，只有分母文件数看得见；改 include 必须同批改这个数字）；② `Warn on attribution gap (PR)` 用 `diff-cover.json` 的 `src_stats` 与「改动文件 ∩ lcov 有记录」做集合比对并**点名文件**，不再匹配 diff-cover 的 markdown 文案（升级改词即永久失配）也不再自己重算分母路径（目录改名时 `git diff` 不报错、added=0 会静默变哑）；③ `Append diff coverage summary` 在报告缺失时输出「未生成」而非静默 0 行。三者的结构断言与反例集在 `scripts/tests/test_ci_frontend_coverage.py`（改判据必须连带改那里）。
- **vitest 系必须同版一起升**（#486）：`vitest` 与 `@vitest/coverage-v8` 在 `package.json` 里**都精确钉**同一版本（不带 `^`）。这不是风格选择而是上游事实——两者的 peer 互为精确锚（`@vitest/coverage-v8@X` 的 peer 是 `vitest: X`，反之亦然）；一精一 caret 时任何把 vitest 解析抬过钉版的操作（`npm update`、`npm i vitest@latest`、删 lock 重装）会撞 `ERESOLVE`，或 nest 出第二份 vitest，使**覆盖率数据与 CLI 版本分叉**（分母口径静默变一套、断言跑在另一套）。升版 = 两包同时改 + 先跑 `npm run test`（#402「工具链升级先跑其单测」的同源约束，此处是版本侧）。
