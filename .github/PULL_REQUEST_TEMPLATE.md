...
- [ ] **数值口径**：涉及金额/份额/净值时，量化只在产生点、走统一入口、舍入模式与 §2.11 一致，无 `float()` 掉标度
- [ ] **契约同步**：错误码清单 / AGENTS / runbook / visual-spec 已随代码更新（无守门的产物，不适用请注明）
...
- [ ] 前端 E2E（CI frontend-e2e）；改动表格/图表列结构时已跑 `scripts/visual-verify.sh` 目检
