...
<!-- CI 的 `Assert review trace` 步骤（issue #560）按本节判红，合规最低要求只有一句话：
     正文**任意位置**有一行「结论：…」且冒号后非空白（不要求落在本小节内）；未做独立 L2
     语义审查也须照实写——判绿的是「有信息」，不是「审过」。改动超过 15 个文件时，须在
     正文写明规模或不拆分的理由（规模护栏见[审查规范](../docs/reference/code-review.md) §5）。
     门禁只认**你写下的可见正文**：本注释、以及原样保留的模板占位行/复选框文案都不算留痕。 -->
